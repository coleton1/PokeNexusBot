            ...
            for image_path, image in images:
                if find_image_on_screen(image):
                    if image_path == 'battle.png':
                        ...
                        break
                    elif image_path == 'haunter.png':  #DOES NOT WORK 
...
def login_gui():
    ...
    root3.title("NexusBot Login")

    emptyspace = ctk.CTkLabel(root3,text="")
    emptyspace.pack(pady=13)

    Logintitle = ctk.CTkLabel(root3,text="Login",font=("Arial",26,"bold"))
    ...
    login_button = ctk.CTkButton(root3,text="Submit")
...
#PLS TEST IF DETECT SCREEN FUNCTION WORKS ON PC 

#possibly make log in gui????????????
